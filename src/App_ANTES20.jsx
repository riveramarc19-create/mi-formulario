...
export default function App() {
  ...
  // --- MATCH INTELIGENTE DE CASERÍO ---
  // Normaliza un texto: mayúsculas, sin tildes, sin puntos, espacios colapsados.
  const normalizarCaserio = (txt) => (txt || '')
      .toString().toUpperCase()
      .normalize('NFD').replace(/[\u0300-\u036f]/g, '') // quita tildes
      .replace(/[.]/g, '').replace(/\s+/g, ' ').trim();

  // Dada una dirección libre del HIS, intenta resolverla contra la lista oficial.
  // Devuelve { exacto } si coincide tras normalizar, o { sugerido } si se parece mucho, o {} si nada.
  const resolverCaserio = useMemo(() => (direccionLibre) => {
      const objetivo = normalizarCaserio(direccionLibre);
      if (!objetivo) return {};
      // 1. Coincidencia exacta directa (ya está tal cual en la lista)
      if (listaCaserios.includes(direccionLibre)) return { exacto: direccionLibre };
      // 2. Coincidencia exacta tras normalizar (tildes/puntos/espacios)
      const exactoNorm = listaCaserios.find(c => normalizarCaserio(c) === objetivo);
      if (exactoNorm) return { exacto: exactoNorm };
      // 3. Coincidencia por inclusión (uno contiene al otro), útil para abreviaturas
      const incluye = listaCaserios.find(c => {
          const n = normalizarCaserio(c);
          return n.includes(objetivo) || objetivo.includes(n);
      });
      if (incluye) return { sugerido: incluye };
      // 4. Coincidencia por primera palabra (ej. "SAN LAZARO ALTO" -> "SAN LAZARO")
      const primeraPalabra = objetivo.split(' ')[0];
      if (primeraPalabra.length >= 3) {
          const porInicio = listaCaserios.find(c => normalizarCaserio(c).startsWith(primeraPalabra));
          if (porInicio) return { sugerido: porInicio };
      }
      // 5. Coincidencia por palabra distintiva compartida (ej. "S. LAZARO" -> "SAN LAZARO")
      //    Compara la palabra más larga del objetivo contra las palabras de cada caserío.
      const palabrasObj = objetivo.split(' ').filter(w => w.length >= 4);
      if (palabrasObj.length) {
          const porPalabra = listaCaserios.find(c => {
              const palabrasC = normalizarCaserio(c).split(' ');
              return palabrasObj.some(w => palabrasC.includes(w));
          });
          if (porPalabra) return { sugerido: porPalabra };
      }
      return {};
  }, [listaCaserios]);
  ...
  // --- BOTÓN LÁPIZ DE EDICIÓN INDIVIDUAL ---
  ...
  const BotonEditarCampo = ({ campo, color = "blue" }) => {
      ...
      const colores = {
          blue:    activo ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-600 hover:bg-blue-200',
          emerald: activo ? 'bg-emerald-600 text-white' : 'bg-emerald-100 text-emerald-600 hover:bg-emerald-200',
          indigo:  activo ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-600 hover:bg-indigo-200',
      };
      return (
          <button
              type="button"
              onMouseDown={(e) => { e.preventDefault(); toggleCampoEditable(campo); }}
              title={activo ? 'Bloquear este campo' : 'Editar este campo'}
              className={`absolute right-1.5 top-1/2 -translate-y-1/2 z-20 p-1 rounded-md shadow-sm transition-all active:scale-90 ${colores[color]}`}
          >
              {activo ? <Lock size={12} strokeWidth={2.5}/> : <Edit size={12} strokeWidth={2.5}/>}
          </button>
      );
  };
  ...
  // Cierra la lista flotante de caserío si se hace scroll (la posición fixed quedaría desalineada)
  useEffect(() => {
      if (!showCaserioList) return;
      const cerrar = () => { setShowCaserioList(false); setCaserioQuery(""); };
 ...
 // --- NUEVA LÓGICA PROACTIVA: DETECTAR GESTANTE POR FUR (CORREGIDO) ---
  ...
  const selectPatient = (p) => {
      ...
      // --- RESOLVER CASERÍO (Propuesta D): guardar el texto original del HIS como referencia ---
      // Si la dirección del HIS coincide (exacta o normalizada) con un caserío válido, se usará;
      // si solo se parece, se guarda como pista/sugerencia visible bajo el campo.
      const resol = resolverCaserio(p.direccion);
      // Guarda el texto original solo si NO quedó como valor exacto (para mostrarlo como referencia)
      setCaserioOriginalHIS(resol.exacto ? "" : (p.direccion ? String(p.direccion).trim() : ""));
      setCaserioQuery("");
      setShowCaserioList(false);
      setCamposEditables({}); // reiniciar desbloqueos individuales para el nuevo paciente
      ...
      if (isSameJurisdiction || isActivityAPP) {
          ...
          setPatientData(prev => ({ 
              ...
              direccion: resolverCaserio(p.direccion).exacto || '', 
              distrito: p.distrito || '', 
  ...
  const handleNextStep = () => {
    ...
    if (step === 1) {
	 ...
	 const financiadoresValidos = ["1-USUARIO", "2-SIS", "3-ESSALUD", "10-OTROS"];
        ...
        // Dirección: debe ser un caserío válido de la lista oficial (no se acepta texto libre)
        if (!patientData.direccion || !listaCaserios.includes(patientData.direccion)) { 
            alert("⚠️ CASERÍO INVÁLIDO\nSeleccione un caserío de la lista escribiendo sus primeras letras."); 
    ...
    try {
		...
		if (isFirst) {
		    ...
		    const wFURLabel = w.lab * 2;
                ...
                cx = mx + w.idx;
                cell(isFirst && p.fecAtencion ? p.fecAtencion.split('-')[2] : "", cx, yData, w.dia, hDataBlock, {align:'center', vAlign:'middle', fontSize:7, bold:false}); 
                cx += w.dia;
                cell(isFirst ? p.dni : "", cx, yData, w.dni, hRowData, {align:'center', fontSize:7, bold:true});
                cell(isFirst ? p.hc : "", cx, yData + hRowData, w.dni, hRowData, {align:'center', fontSize:7, bold:true});
		...
		// --- LÓGICA COLOR MORADO PARA CONDICIÓN ---
                ...
                if (isFirst && (p.condicion === 'GESTANTE' || p.condicion === 'PUERPERA')) {
                    colorCondicion = [204, 153, 255]; // Morado
                }
                cell(isFirst ? p.condicion : "", cx, yData + (hRowData*2), w.dni, hRowData, {align:'center', fontSize:5.5, bold: !!colorCondicion, fill: colorCondicion});
                cx += w.dni;
                ...
                cell(isFirst ? codigoFinanciador : "", cx, yData, w.fin, hDataBlock, {align:'center', vAlign:'middle', fontSize:7, bold:false});
                cx += w.fin;
                cell(isFirst ? p.distrito : "", cx, yData, w.dist, hRowData, {align:'left', fontSize:5, bold:false});
                let direccionTexto = p.direccion || "";
                if(p.centroPoblado) direccionTexto += " " + p.centroPoblado;
                
                cell(isFirst ? direccionTexto : "", cx, yData + hRowData, w.dist, hRowData * 2, {
                    align:'left', 
                    fontSize:5, 
                    vAlign:'top', 
                    bold:false, 
                ...
                cell(isFirst && a.y ? a.y + " años" : "", cx, yData, w.edad, hRowData, {align:'center', fontSize: 6, bold:true});
                cell(isFirst && a.m ? a.m + " meses" : "", cx, yData + hRowData, w.edad, hRowData, {align:'center', fontSize: 6, bold:true});
                cell(isFirst && a.d ? a.d + " días" : "", cx, yData + (hRowData*2), w.edad, hRowData, {align:'center', fontSize: 6, bold:true});
                cx += w.edad;

                cell(isFirst ? p.sexo : "", cx, yData, w.sex, hDataBlock, {align:'center', vAlign:'middle', fontSize:7, bold:false});
                cx += w.sex;
                ...
                cx += (w.antL*2 + w.antV*2);
                cell(isFirst ? p.condEst : "", cx, yData, w.est, hDataBlock, {align:'center', vAlign:'middle', bold:false, fontSize:7}); cx += w.est;
                cell(isFirst ? p.condServ : "", cx, yData, w.serv, hDataBlock, {align:'center', vAlign:'middle', bold:false, fontSize:7}); cx += w.serv;
             ...
             for(let k = 0; k < chunks.length; k++) {
                ...
                for(let i = 0; i < 3; i++) {
                    ...
                    if (!isEmpty) {
                        ...
                        if (i === 0) { row[1] = (k===0 && p.fecAtencion) ? p.fecAtencion.split('-')[2] : "";
                        row[2] = (k===0 ? p.dni : ''); row[3] = (k===0 ? (p.financiador === 'SIS' ? '1' : '2') : ''); row[4] = (k===0 ? p.distrito : '');
                        row[5] = (k===0 && (age.y || age.y === 0)) ? `${age.y} años` : ''; row[6] = (k===0 ? p.sexo : ''); row[7] = "Talla";
                        row[8] = (k===0 ? parseFloat(c.talla)||'' : ''); row[9] = "P.C."; row[10] = (k===0 ? parseFloat(c.pCef)||'' : ''); row[11] = (k===0 ? p.condEst : '');
                        row[12] = (k===0 ? p.condServ : ''); } 
                        else if (i === 1) { row[2] = (k===0 ? p.hc : '');
                        row[4] = (k===0 ? p.direccion : ''); row[5] = (k===0 && (age.m || age.m === 0)) ? `${age.m} meses` : ''; row[7] = "Peso";
                        row[8] = (k===0 ? parseFloat(c.peso)||'' : ''); row[9] = "P.Abd"; row[10] = (k===0 ? parseFloat(c.pAbd)||'' : '');
                        } 
                        else if (i === 2) { row[2] = (k===0 ? (p.condicion || "-") : ''); row[4] = (k===0 ? "CENTRO POBLADO" : ''); row[5] = (k===0 && (age.d || age.d === 0)) ? `${age.d} días` : ''; row[7] = "HB";
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  ...
                  <input 
                    ...
                    className="w-full pl-12 pr-4 py-3.5 rounded-2xl bg-slate-50 border-2 border-slate-100 outline-none text-base font-bold text-slate-700 transition-all focus:border-purple-500 focus:bg-white focus:ring-4 focus:ring-purple-50 shadow-sm placeholder:text-slate-300"
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  ...
                  <input 
                    ...
                    className="w-full pl-12 pr-12 py-3.5 rounded-2xl bg-slate-50 border-2 border-slate-100 outline-none text-base font-bold text-slate-700 transition-all focus:border-purple-500 focus:bg-white focus:ring-4 focus:ring-purple-50 shadow-sm placeholder:text-slate-300 font-mono"
                    placeholder="••••••••"
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-purple-600 transition-colors p-1">
              ...
              <label className="flex items-center gap-2.5 cursor-pointer select-none ml-1">
                <input 
                  ...
                  className="w-[18px] h-[18px] rounded cursor-pointer accent-purple-600"
                />
              ...
              <button 
                ...
                style={{background: 'linear-gradient(135deg, #6D28D9 0%, #5B21B6 40%, #047857 60%, #059669 100%)', boxShadow: '0 4px 15px rgba(109,40,217,0.3), 0 2px 8px rgba(5,150,105,0.25)'}}
              >
                    ...
                    <div>
                        ...
                        <select 
                            ...
                            className={(adminData.mes 
                                ...
                                : cfgInputStyle.replace('border-slate-200', 'border-red-400').replace('bg-white', 'bg-red-50') + ' animate-pulse text-red-600') + ' text-xs'} 
                            value={adminData.mes} 
                            onChange={handleAdmin}
                        >
                            <option value="" disabled className="text-xs"> SELECCIONE MES </option>
            ...
            {patientData.paciente && step < 4 && (
              ...
              `}>
                 <div className="flex items-center gap-3 min-w-0">
                    {/* ÍCONO DINÁMICO */}
                    <div className={`
                        p-1.5 rounded-lg shadow-sm border border-white/50 shrink-0
                    ...
                    {/* DATOS DE TEXTO — NOMBRE Y DATOS EN UNA SOLA LÍNEA */}
                    <div className="flex items-center gap-3 min-w-0 flex-wrap">
                       <h3 className={`text-xs font-black leading-none tracking-wide uppercase whitespace-nowrap
                       ...
                       <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wide opacity-80 whitespace-nowrap">
                 ...
                 {/* INDICADOR H. CLÍNICA — EN LÍNEA Y COMPACTO */}
                 <div className="text-right hidden sm:flex items-baseline gap-1.5 shrink-0">
                    <span className={`text-[9px] font-bold uppercase tracking-wider ${(patientData.sexo === 'M' || patientData.sexo === 'MASCULINO') ? 'text-sky-600' : (patientData.sexo === 'F' || patientData.sexo === 'FEMENINO') ? 'text-pink-600' : 'text-slate-400'}`}>H. Clínica</span>
                    <span className={`text-sm font-black leading-none ${(patientData.sexo === 'M' || patientData.sexo === 'MASCULINO') ? 'text-sky-900' : (patientData.sexo === 'F' || patientData.sexo === 'FEMENINO') ? 'text-pink-900' : 'text-slate-700'}`}>
                        {patientData.hc || "---"}
                    </span>
                 </div>
              </div>
...
{validationAlert.isOpen && (
  <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 animate-in fade-in duration-200">
    <div className="absolute inset-0 bg-black/70 backdrop-blur-md" onClick={() => setValidationAlert({ ...validationAlert, isOpen: false })}></div>

    {/* GLASS OSCURO PREMIUM — acento de color según tipo de alerta */}
    <div className={`relative rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border animate-in zoom-in duration-300
        ${validationAlert.type === 'GESTANTE' ? 'border-pink-400/40' : validationAlert.type === 'TRANSFER' ? 'border-indigo-400/40' : 'border-orange-400/40'}`}
        style={{ background: 'linear-gradient(135deg, rgba(30,24,20,0.92), rgba(42,32,26,0.92))', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }}>
      
      {/* CABECERA */}
      <div className={`px-8 py-6 border-b flex items-center gap-4
          ${validationAlert.type === 'GESTANTE' ? 'border-pink-400/20' : validationAlert.type === 'TRANSFER' ? 'border-indigo-400/20' : 'border-orange-400/20'}`}>
        
        <div className={`p-4 rounded-full shrink-0 border
            ${validationAlert.type === 'GESTANTE' ? 'bg-pink-500/20 text-pink-300 border-pink-400/30' : validationAlert.type === 'TRANSFER' ? 'bg-indigo-500/20 text-indigo-300 border-indigo-400/30' : 'bg-orange-500/20 text-orange-300 border-orange-400/30'}`}>
        ...
        <div>
          <h3 className={`text-2xl font-black
              ${validationAlert.type === 'GESTANTE' ? 'text-pink-200' : validationAlert.type === 'TRANSFER' ? 'text-indigo-200' : 'text-orange-200'}`}>
      ...
      <div className="p-8 max-h-[60vh] overflow-y-auto">
        <p className="text-slate-100 font-bold text-lg mb-6 leading-snug">{validationAlert.message}</p>
        <div className={`p-5 rounded-2xl border flex items-start gap-4
            ${validationAlert.type === 'GESTANTE' ? 'bg-pink-500/10 border-pink-400/30' : validationAlert.type === 'TRANSFER' ? 'bg-indigo-500/10 border-indigo-400/30' : 'bg-orange-500/10 border-orange-400/30'}`}>
          <div className={`mt-1
              ${validationAlert.type === 'GESTANTE' ? 'text-pink-300' : validationAlert.type === 'TRANSFER' ? 'text-indigo-300' : 'text-orange-300'}`}>
              <AlertTriangle size={24}/>
          </div>
          <div className="text-sm font-medium text-slate-300 whitespace-pre-line leading-relaxed">{validationAlert.details}</div>
        </div>
      </div>

      {/* BOTÓN */}
      <div className="px-8 py-5 bg-black/20 border-t border-white/10 flex justify-end">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                                    ...
                                    <div className="relative group">
                                        ...
                                        <input 
                                            ...
                                            onChange={handlePatient}
                                            readOnly={esSoloLectura('hc')}
                                            className={`w-full h-9 rounded-lg border-2 px-2 pr-8 font-black text-base outline-none bg-white shadow-sm transition-all
                                        ...
                                        />
                                        <BotonEditarCampo campo="hc" color="blue" />
                                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                                    ...
                                    <div className="relative group md:col-span-3">
                                        ...
                                        <input 
                                            ...
                                            onChange={handlePatient}
                                            readOnly={esSoloLectura('distrito')}
                                            className="w-full bg-white h-9 rounded-lg border-2 border-blue-200 px-2 pr-8 font-bold text-blue-800 outline-none text-xs shadow-sm focus:border-blue-500 transition-all uppercase"
                                        />
                                        <BotonEditarCampo campo="distrito" color="blue" />
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div className="bg-emerald-100/80 rounded-2xl p-3 border-2 border-emerald-300 shadow-md relative overflow-hidden">
                            ...
                            <div className="space-y-2 relative z-10">
                                {/* AUTOCOMPLETE INTELIGENTE DE CASERÍO
                                    - Se escribe para filtrar, pero SOLO se acepta un valor de la lista oficial.
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        onFocus={() => { if(!esSoloLectura('direccion')){ 
                                            ...
                                            if(caserioInputRef.current){ const r = caserioInputRef.current.getBoundingClientRect(); setCaserioDropPos({top: r.bottom + 4, left: r.left, width: r.width}); }
                                        } }}
                                        onChange={(e) => {
                                        ...
                                        onBlur={() => {
                                            // Al salir, si lo escrito no es un caserío válido, se descarta (no se permite texto libre)
                                    ...
                                    {/* LISTA DE SUGERENCIAS FLOTANTE (fixed: escapa de cualquier overflow-hidden) */}
                                    {showCaserioList && !esSoloLectura('direccion') && caserioDropPos && (() => {
                                        ...
                                        return (
                                            <div 
                                                style={{ position: 'fixed', top: caserioDropPos.top, left: caserioDropPos.left, width: caserioDropPos.width }}
                                                className="bg-white border-2 border-emerald-300 rounded-lg shadow-2xl max-h-72 overflow-y-auto z-[9999]"
                                            >
                                ...
                                {caserioOriginalHIS && !esSoloLectura('direccion') && !(patientData.direccion && listaCaserios.includes(patientData.direccion)) && (() => {
                                    const sugerido = resolverCaserio(caserioOriginalHIS).sugerido;
                                    return (
                                        <div className="bg-amber-50 border border-amber-300 rounded-lg px-2.5 py-1.5 flex items-center justify-between gap-2">
                                            <div className="min-w-0">
                                                <span className="text-[8px] font-bold text-amber-600 uppercase tracking-wide block leading-none">Registrado en HIS</span>
                                                <span className="text-[11px] font-black text-amber-900 uppercase truncate block leading-tight" title={caserioOriginalHIS}>{caserioOriginalHIS}</span>
                                            </div>
                                            {sugerido ? (
                                                <button
                                                    type="button"
                                                    onMouseDown={(e) => {
                                                        e.preventDefault();
                                                        setPatientData(prev => ({...prev, direccion: sugerido}));
                                                        setCaserioOriginalHIS("");
                                                        setCamposEditables(prev => ({ ...prev, direccion: false }));
                                                    }}
                                                    className="shrink-0 bg-emerald-600 text-white text-[10px] font-black px-2.5 py-1 rounded-md hover:bg-emerald-700 transition-colors uppercase whitespace-nowrap"
                                                    title={`Usar "${sugerido}"`}
                                                >
                                                    ¿Usar {sugerido}?
                                                </button>
                                            ) : (
                                                <span className="shrink-0 text-[9px] font-bold text-amber-500 uppercase whitespace-nowrap">Escriba el caserío ▲</span>
                                            )}
                                        </div>
                                    );
                                })()}
                            ...
                            <div className="space-y-2 relative z-10">
                                <div className="relative">
                                    <select 
                                        ...
                                        onChange={handlePatient}
                                        disabled={esSoloLectura('estOrigen')}
                                        className={`w-full h-8 px-2 pr-8 rounded-lg border-2 border-indigo-200 font-bold text-xs text-indigo-900 outline-none transition-all shadow-sm
                                            ${esSoloLectura('estOrigen') ? 'bg-slate-100 cursor-not-allowed opacity-80' : 'bg-white cursor-pointer hover:border-indigo-400'}`}
                                    >
                                    ...
                                    <BotonEditarCampo campo="estOrigen" color="indigo" />
                                </div>
                                 <div className="flex justify-between items-center px-1 pt-1">
                                     <span className="text-[11px] font-bold text-slate-400">{isPatientDataLocked ? '🔒 Datos protegidos' : '✏️ Edición libre activa'}</span>
                                     <button onClick={() => { setIsPatientDataLocked(!isPatientDataLocked); setCamposEditables({}); }} className={`text-[11px] font-black px-3 py-1 rounded-lg flex items-center gap-1.5 transition-all shadow-sm active:scale-95 ${isPatientDataLocked ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-emerald-600 text-white hover:bg-emerald-700'}`}>
                                         {isPatientDataLocked ? <><Edit size={12} strokeWidth={2.5}/> EDITAR TODO</> : <><Lock size={12} strokeWidth={2.5}/> BLOQUEAR TODO</>}
                                     </button>
                                 </div>
                            </div>
                        </div>
