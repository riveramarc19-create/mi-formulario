...
import { Activity, ArrowRight, ArrowLeft, X, Plus, LogOut, Database, Search, Trash2, FileSpreadsheet, Calendar, Edit, Download, Grid, Save, UserPlus, Users, Save as SaveIcon, FileText, AlertTriangle, Calculator, Siren, Baby, RefreshCw, CheckCircle, Droplets, Eye, EyeOff, Heart, Syringe, Brain, Smile } from 'lucide-react';
...
export default function App() {
  ...
  const selectPatient = (p) => {
      ...
      setTimeout(() => { 
          // 1. Verificamos si tiene dirección
          ...
          // LÓGICA: Si NO tiene dirección, NO bloqueamos (dejamos editar). Si TIENE, bloqueamos.
  ...
  const handleNextStep = () => {
    const markError = (name) => {
        ...
        // Selector genérico para input y select
        if (input) {
            ...
            // Para selects también escuchamos el cambio
    ...
    // ========================================================================
    // PASO 1: VALIDACIONES ADMINISTRATIVAS (ESTRICTAS)
    // ========================================================================
    if (step === 1) {
        // 1. Identidad
        if (!patientData.dni || !patientData.paciente) {
            alert("⚠️ FALTAN DATOS\nComplete DNI y Nombre.");
        ...
        // 3. CONDICIÓN ESTABLECIMIENTO Y SERVICIO (RECUPERADO)
        ...
        // 4. HISTORIA CLÍNICA (NO PUEDE SER IGUAL AL DNI)
        const dniStr = String(patientData.dni).trim();
        const hcStr = patientData.hc ? String(patientData.hc).trim() : "";
        
        // Comparación Numérica para detectar 00373661 == 373661
        const dniNum = parseInt(dniStr, 10);
        const hcNum = parseInt(hcStr, 10);

        if (!hcStr) {
            alert("⚠️ FALTA HISTORIA CLÍNICA\nIngrese el N° de Carpeta.");
            markError("hc"); return;
        }

        if (dniStr === hcStr || (dniNum === hcNum && dniNum > 0)) {
            alert(`⛔ ERROR DE HISTORIA CLÍNICA\nLa H.C. (${hcStr}) es igual al DNI del paciente.\nEsto es incorrecto. Ingrese el N° DE CARPETA.`);
            markError("hc"); return;
        }

               // Caso 1: Si es NUEVO en Establecimiento, OBLIGATORIAMENTE debe ser NUEVO en Servicio.
        ...
        // Caso 2: Si es REINGRESANTE en Establecimiento, NO puede ser CONTINUADOR en Servicio.
        // (Un reingresante es alguien que vuelve después de tiempo, por lo que su cuenta en el servicio se reinicia o es Reingresante también, pero no Continuador inmediato).
    ...
    // ========================================================================
    // PASO 2: VALIDACIÓN MÉDICA INTELIGENTE (NO TOCADO - SE MANTIENE IGUAL)
    // ========================================================================
    if (step === 2) {
      ...
      // RECIÉN NACIDO
      if (y === 0 && m === 0 && d >= 0) {
          if (t && (t < 40 || t > 62)) { alert(`⛔ TALLA IRREAL (Neonato)\nRango: 40-62cm. Valor: ${t}`);
          markError("talla"); return; }
          if (p && (p < 1.5 || p > 6)) { alert(`⛔ PESO IRREAL (Neonato)\nRango: 1.5-6kg. Valor: ${p}`);
          markError("peso"); return; }
      }
      // LACTANTE
      else if (y === 0 && m > 0) {
        if (t && (t < 45 || t > 85)) { alert(`⛔ TALLA IRREAL (Lactante)\nRango: 45-85cm. Valor: ${t}`);
        markError("talla"); return; }
        if (p && (p < 2.5 || p > 14)) { alert(`⛔ PESO IRREAL (Lactante)\nRango: 2.5-14kg. Valor: ${p}`);
        markError("peso"); return; }
      }
      // NIÑO
      else if (y >= 1 && y < 5) {
        if (t && (t < 65 || t > 120)) { alert(`⛔ TALLA IRREAL (Niño ${y}a)\nRango: 65-120cm. Valor: ${t}`);
        markError("talla"); return; }
        if (p && (p < 7 || p > 28)) { alert(`⛔ PESO IRREAL (Niño ${y}a)\nRango: 7-28kg. Valor: ${p}`);
        markError("peso"); return; }
      }
      // ESCOLAR/ADOLESCENTE
      else if (y >= 5 && y < 18) {
        if (t && (t < 95 || t > 210)) { alert(`⛔ TALLA IRREAL (${y} años)\nValor: ${t}cm`);
        markError("talla"); return; }
        if (p && (p < 12 || p > 130)) { alert(`⛔ PESO IRREAL (${y} años)\nMínimo ~12kg. Valor: ${p}`);
        markError("peso"); return; }
      }
      // ADULTO
      else if (y >= 18) {
        if (t && (t < 130 || t > 250)) { alert(`⛔ TALLA INVÁLIDA (Adulto)\nValor: ${t} cm`);
        markError("talla"); return; }
        if (p && (p < 35 || p > 300)) { alert(`⛔ PESO INVÁLIDO (Adulto)\nValor: ${p} kg`);
        markError("peso"); return; }
      }

      if (hb && (hb < 3 || hb > 24)) { alert("⛔ HEMOGLOBINA ERRÓNEA (3-24)");
      markError("hb"); return; }
      if (pAbd && (parseFloat(pAbd) < 10 || parseFloat(pAbd) > 200)) { alert("⛔ P. ABDOMINAL ERRÓNEO");
      markError("pAbd"); return; }
    }

    // ========================================================================
    // PASO 3: VALIDACIÓN CIE-10
    // ========================================================================
    if (step === 3) {
    ...
    <div className="h-screen w-full bg-slate-100 font-sans flex flex-col overflow-hidden">
      ...
      <div className="flex-1 w-full flex items-center justify-center p-4 relative bg-slate-100">
        ...
        (
          ...
          <div className="w-full h-full flex flex-col items-center overflow-y-auto py-6 gap-6 animate-in fade-in zoom-in duration-300">
                ...
                <div className="flex flex-wrap gap-4 justify-center">
                    {[
                        ...
                        <a 
                            ...
                            className="group flex flex-col items-center justify-center w-32 h-32 bg-slate-50 border-2 border-slate-100 rounded-2xl hover:bg-red-50 hover:border-red-200 transition-all cursor-pointer shadow-sm hover:shadow-md hover:-translate-y-1 text-center p-2 no-underline"
          ...
          <div className="bg-white rounded-[30px] shadow-2xl border border-slate-200 overflow-hidden flex flex-col w-full max-w-[95%] h-[90vh] animate-in zoom-in-95 duration-200 ring-1 ring-black/5 m-auto">
            ...
            <div className="flex border-b border-slate-100 bg-white px-6 shrink-0 justify-between items-center">
              <div className="flex-1 flex">
                  {['Paciente', 'Datos Clínicos', 'Diagnósticos', 'Finalizar'].map((label, idx) => (
                      <div key={idx} className={`flex-1 text-center py-4 text-xs font-bold border-b-[3px] transition-colors ${step === idx + 1 ?
            ...
            <div className="flex-grow overflow-y-auto p-6 bg-[#FAFAFA] no-scrollbar relative" ref={dxListRef}>
                ...
                <div className="absolute inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-300">
                    <div className={`rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border-2 animate-in zoom-in duration-300 bg-white ${validationAlert.type === 'GESTANTE' ? 'border-pink-200' : 'border-orange-200'}`}>
                        <div className={`px-8 py-6 border-b flex items-center gap-4 ${validationAlert.type === 'GESTANTE' ? 'bg-pink-50 border-pink-100' : 'bg-orange-50 border-orange-100'}`}>
                            <div className={`p-4 rounded-full shrink-0 shadow-sm ${validationAlert.type === 'GESTANTE' ?
                            'bg-pink-200 text-pink-700' : 'bg-orange-200 text-orange-700'}`}>
                                {validationAlert.type === 'GESTANTE' ?
                                <Baby size={40} strokeWidth={2} /> : <Activity size={40} strokeWidth={2} />}
                            </div>
                            <div>
                                <h3 className={`text-2xl font-black ${validationAlert.type === 'GESTANTE' ? 'text-pink-700' : 'text-orange-800'}`}>{validationAlert.title}</h3>
                                <p className={`text-xs font-bold mt-1 uppercase ${validationAlert.type === 'GESTANTE' ?
                                'text-pink-400' : 'text-orange-400'}`}>Corrección requerida</p>
                            </div>
                        </div>
                        <div className="p-8">
                            <p className="text-slate-700 font-bold text-lg mb-6 leading-snug">{validationAlert.message}</p>
                            <div className={`p-5 rounded-2xl border flex items-start gap-4 shadow-sm ${validationAlert.type === 'GESTANTE' ?
                            'bg-pink-50 border-pink-200' : 'bg-orange-50 border-orange-200'}`}>
                                <div className={`mt-1 ${validationAlert.type === 'GESTANTE' ?
                                'text-pink-500' : 'text-orange-500'}`}><AlertTriangle size={24}/></div>
                        ...
                        <div className="px-8 py-5 bg-slate-50 border-t border-slate-100 flex justify-end">
                            <button onClick={() => { setValidationAlert({ ...validationAlert, isOpen: false });
                            if (validationAlert.type === 'LAB_MISSING') { setStep(2); setShowHbError(true); } }} className={`px-8 py-3 rounded-xl font-black shadow-lg transition-transform active:scale-95 text-sm tracking-wide text-white ${validationAlert.type === 'GESTANTE' ?
                            'bg-pink-500 hover:bg-pink-600' : 'bg-orange-500 hover:bg-orange-600'}`}>ENTENDIDO, VOY A CORREGIR</button>
                        </div>
                    </div>
                </div>
              )}
              ...
              {/* PASO 1: PACIENTE */}
              {step === 1 && (
                <div className="animate-in fade-in slide-in-from-right-8 duration-300 min-w-[800px] mx-auto max-w-5xl relative">
                  <div className={`border rounded-xl p-4 mb-4 shadow-sm relative z-50 transition-all duration-300 ${showSuggestions ? 'bg-white ring-4 ring-blue-50 border-blue-200 shadow-xl' : getTheme(1).bgLight + ' ' + getTheme(1).border}`} ref={searchRef}>
                          ...
                          <label className={`text-[10px] font-bold uppercase block mb-1.5 ml-2 ${getTheme(1).labelText}`}>Buscar Paciente (DNI o Nombre)</label>
                            <div className="relative flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    {showSuggestions && (
                                        <div className="absolute top-14 left-0 w-full bg-white rounded-2xl shadow-2xl border border-slate-200 max-h-[350px] overflow-y-auto p-2 no-scrollbar animate-in zoom-in-95 duration-200">
                                            {suggestions.map((p, idx) => {
                                                const estConfigRaw = adminData.establecimiento.trim().toUpperCase();
                                                const estPatientRaw = p.estOrigen ? p.estOrigen.trim().toUpperCase() : "";
                                                const isMatch = estPatientRaw.includes(cleanStr(estConfigRaw)) || cleanStr(estConfigRaw).includes(cleanStr(estPatientRaw));
                                                return (
                                                    <div key={idx} onMouseDown={() => selectPatient(p)} className="p-3 cursor-pointer rounded-xl group hover:bg-blue-50 transition-all border-b border-slate-50 last:border-0 flex items-start gap-3">
                                                        <div className={`mt-1 p-2 rounded-full ${isMatch ? 'bg-slate-100 text-slate-400' : 'bg-amber-100 text-amber-500'}`}><Users size={18} /></div>
                                                        <div className="flex-1">
                                                            <div className="font-bold text-sm text-slate-700 group-hover:text-blue-700 uppercase">{getHighlightedText(p.nombre, searchTerm)}</div>
                                                            <div className="flex flex-wrap gap-2 mt-1.5 text-[10px] font-bold uppercase">
                                                                <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded border border-slate-200">DNI: <span className="text-slate-700">{getHighlightedText(p.dni, searchTerm)}</span></span>
                                                                <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded border border-slate-200">HC: {p.hc}</span>
                                                                <span className={`px-2 py-0.5 rounded border flex items-center gap-1 ${isMatch ?
                                                                'bg-slate-100 text-slate-500 border-slate-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>{!isMatch && <AlertTriangle size={10} className="text-amber-500"/>} Est: {p.estOrigen || 'NO REGISTRADO'}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                );
                  ...
                  <div className={`transition-all duration-500 ease-in-out ${showSuggestions ?
                  'opacity-40 blur-sm pointer-events-none grayscale-[0.5]' : 'opacity-100 blur-0'}`}>
                      <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 space-y-4">
                        ...
                        <div className="grid grid-cols-12 gap-2 items-end border-b pb-4 border-slate-100 mb-2 bg-slate-50/50 p-2 rounded-xl">
                            <div className="col-span-2 flex flex-col gap-1"><label className={getLabelStyle(1)}>Cond. Est.</label><select name="condEst" className={getSelectStyle(patientData.condEst)} value={patientData.condEst} onChange={handlePatient}><option value="">...</option><option value="N">NUEVO</option><option value="C">CONTINUADOR</option><option value="R">REINGRESANTE</option></select></div>
                            <div className="col-span-2 flex flex-col gap-1"><label className={getLabelStyle(1)}>Cond. Serv.</label><select name="condServ" className={getSelectStyle(patientData.condServ, isInvalidCombo)} value={patientData.condServ} onChange={handlePatient}><option value="">...</option><option value="N">NUEVO</option><option value="C">CONTINUADOR</option><option value="R">REINGRESANTE</option></select></div>
                            <div className="col-span-2 flex flex-col gap-1"><label className={getLabelStyle(1)}>Condición</label><select name="condicion" className={getInputStyle(1) + " text-xs"} value={patientData.condicion} onChange={handlePatient}><option value="">NINGUNA</option><option value="GESTANTE">GESTANTE</option><option value="PUERPERA">PUERPERA</option></select></div>
                            <div className="col-span-2 flex flex-col gap-1"><label className={getLabelStyle(1)}>FUR</label><div className="relative"><input type="date" name="fur" className={(patientData.condicion !== 'GESTANTE' ? 'bg-slate-100 opacity-50 cursor-not-allowed' : 'bg-white cursor-pointer') + 
                            " " + getDateInputStyle(1) + " text-center text-xs px-1"} value={patientData.fur} onChange={handlePatient} disabled={patientData.condicion !== 'GESTANTE'} /></div></div>
                            <div className="col-span-4 flex justify-end pb-[2px]">{isPatientDataLocked ?
                            (<button onClick={() => setIsPatientDataLocked(false)} className={`w-full justify-center bg-white border-2 border-slate-200 hover:border-blue-400 text-blue-600 hover:bg-blue-50 px-2 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all shadow-sm h-[38px]`}><Edit size={16}/> EDITAR</button>) : (<button onClick={() => setIsPatientDataLocked(true)} className={`w-full justify-center bg-white border-2 border-red-200 hover:border-red-400 text-red-600 hover:bg-red-50 px-2 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all shadow-sm h-[38px]`}><X size={16}/> BLOQUEAR</button>)}</div>
                        </div>
                        ...
                        <div className="grid grid-cols-12 gap-2 items-end">
                          ...
                          <div className="col-span-2 flex flex-col gap-1"><label className={getLabelStyle(1)}>H. Clínica</label><input ref={hcInputRef} name="hc" className={`${isPatientDataLocked ? getLockedInputStyle(1) : getInputStyle(1)} ${(!patientData.hc || patientData.hc === patientData.dni) && !isPatientDataLocked ?
                          'border-red-500 bg-red-50 focus:border-red-600 focus:ring-red-100' : ''}`} value={patientData.hc} onChange={handlePatient} readOnly={isPatientDataLocked} /></div>
                        ...
                        <div className="grid grid-cols-12 gap-2 items-end">
                          <div className="col-span-8 flex flex-col gap-1"><label className={getLabelStyle(1)}>Paciente</label><input name="paciente" className={isPatientDataLocked ?
                          getLockedInputStyle(1) + " font-black" : getInputStyle(1) + " font-black"} value={patientData.paciente} onChange={handlePatient} readOnly={isPatientDataLocked} /></div>
                          <div className="col-span-4 flex flex-col gap-1"><label className={getLabelStyle(1)}>Distrito</label><input name="distrito" className={isPatientDataLocked ?
                          getLockedInputStyle(1) : getInputStyle(1)} value={patientData.distrito} onChange={handlePatient} readOnly={isPatientDataLocked} /></div>
                        </div>

                        <div className="grid grid-cols-12 gap-2 items-end">
                            <div className="col-span-5 flex flex-col gap-1"><label className={getLabelStyle(1)}>Dirección</label><input name="direccion" className={`${isPatientDataLocked ?
                            getLockedInputStyle(1) : getInputStyle(1)} ${!patientData.direccion ? 'border-red-400 bg-red-50' : ''}`} value={patientData.direccion} onChange={handlePatient} readOnly={isPatientDataLocked} placeholder={!patientData.direccion ? "¡VACÍO!"
                            : ""} /></div>
                            <div className="col-span-3 flex flex-col gap-1"><label className="text-[9px] font-bold text-blue-500 uppercase ml-1 mb-0.5">Seleccionar Caserío</label><select className={`w-full h-9 px-2 rounded-xl border-2 outline-none text-xs font-bold transition-all ${isPatientDataLocked ?
                            'bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed' : 'bg-white border-blue-300 text-blue-700 cursor-pointer hover:border-blue-500 focus:ring-2 focus:ring-blue-100'}`} disabled={isPatientDataLocked} onChange={(e) => { if(e.target.value) { setPatientData(prev => ({...prev, direccion: e.target.value}));
                            } }} value=""><option value="">▼ LISTA CASERÍOS</option>{listaCaserios.map(caserio => ( <option key={caserio} value={caserio}>{caserio}</option> ))}</select></div>
                            <div className="col-span-4 flex flex-col gap-1"><label className={getLabelStyle(1)}>ESTABLECIMIENTO DE ATENCIÓN</label><input name="estOrigen" className={isPatientDataLocked ?
                            getLockedInputStyle(1) : getInputStyle(1)} value={patientData.estOrigen} onChange={handlePatient} readOnly={isPatientDataLocked} /></div>
                        </div>
                      </div>
                  </div>
                </div>
                                  ...
                                  <div key={idx} className="grid grid-cols-12 gap-4 items-center border-b border-slate-50 pb-2 last:border-0 last:pb-0">
                                    <div className="col-span-5">
                                        <div className="flex justify-between items-center mb-1">
                                            ...
                                            {item.specialBtn && !clinicalData[item.name] && !isLocked && ( <button onClick={() => { if(item.name==='pAbd') { setIgnorePAbdValidation(true); setShowPAbdError(false);
...
/>                                       
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
                            ...
                            <div className="flex gap-2 mb-3">
                                <div className="flex-1"><label className="text-[10px] font-bold text-slate-400 block mb-1 uppercase">IMC</label><div className="h-10 bg-slate-100 rounded-lg flex items-center justify-center font-black text-slate-700 border border-slate-200">{clinicalData.imc ||
                                '--'}</div></div>
                                <div className="flex-[1.5]"><label className="text-[10px] font-bold text-slate-400 block mb-1 uppercase">Diagnóstico / Riesgo</label><input name="riesgo" value={clinicalData.riesgo} onChange={handleClinical} className="w-full h-10 bg-white border-2 border-slate-200 rounded-lg px-2 text-xs font-bold outline-none focus:border-teal-500 uppercase text-center" placeholder="NORMAL..." /></div>
                            </div>
                             <button onClick={() => setShowNutriModal(true)} className="w-full border-2 border-dashed border-slate-300 text-slate-500 hover:border-teal-400 hover:text-teal-700 hover:bg-teal-50 font-bold py-2 rounded-xl text-xs transition-all">Ver Tabla Z-Score (OMS)</button>
                        </div>
                    </div>
                  </div>
                </div>
              ...
              {step === 3 && (
                <div className="animate-in fade-in slide-in-from-right-8 duration-300 w-full max-w-[95%] mx-auto">
                   <div className={`flex justify-between items-end mb-6 p-5 rounded-2xl border ${getTheme(3).bgLight} ${getTheme(3).border}`}>
                      <div><h3 className="font-extrabold text-slate-700 text-lg">Diagnósticos CIE-10</h3><p className={`text-xs mt-1 font-medium ${getTheme(3).labelText}`}>Haga clic en las filas para buscar y editar</p></div>
                  </div>
                  <div className="space-y-3">
                      <div className={`grid grid-cols-12 gap-3 text-[10px] font-bold uppercase px-3 items-center ${getTheme(3).labelText}`}><div className="col-span-1 text-center">Opción</div><div className="col-span-6">Diagnóstico Seleccionado</div><div className="col-span-1 text-center">Tipo</div><div className="col-span-2 text-center">Lab</div><div className="col-span-2 text-center">CIE-10</div></div>
              ...
              {step === 4 && (
                <div className="w-full h-full bg-white flex flex-col animate-in fade-in zoom-in duration-300 font-sans">
                  ...
                  <div className="h-10 bg-[#F3F3F3] border-b border-[#E1E1E1] flex items-center px-4 gap-4 shrink-0 text-xs text-slate-600">
                      <div className="flex items-center gap-2"><Grid size={14}/> <span>Vista de Diseño</span></div>
                  ...
                  <div className="flex-1 overflow-auto bg-[#E6E6E6] p-4">
                    <div className="bg-white shadow-xl mx-auto min-w-[1400px] max-w-[1600px] border border-slate-300">
                      ...
                      <div className="flex border-b border-slate-300 bg-[#F8F9FA]">
                        ...
                        {['DÍA','DNI','FIN','DISTRITO','EDAD','SEX','TALLA','PESO','O.ANT','HB','EST','SER','DIAGNÓSTICO','TIPO','LAB','LAB','LAB','CIE'].map((h, i) => (
                                ...
                                return chunks.map((chunk, chunkIdx) => {
                                    ...
                                    return (
                                            <div key={`${idx}-${chunkIdx}`} className="flex border-b border-slate-300 hover:bg-blue-50 transition-colors group">
                                                ...
                                                {/* COLUMNAS 7-10: DATOS CLÍNICOS (VERTICAL) - AUMENTADO A 10PX */}
 ...
 {isFirst && c.hb && <div className="font-bold">Hb:{c.hb}</div>}
                                                ...
                                                {/* --- ZONA DE DIAGNÓSTICOS (3 FILAS INTERNAS) --- */}
