...
{validationAlert.isOpen && (
  <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 animate-in fade-in duration-200">
    <div className="absolute inset-0 bg-amber-900/30 backdrop-blur-sm" onClick={() => setValidationAlert({ ...validationAlert, isOpen: false })}></div>
    ...
    <div className={`relative rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border animate-in zoom-in duration-300
        ${validationAlert.type === 'GESTANTE' ? 'border-pink-300/50' : validationAlert.type === 'TRANSFER' ? 'border-indigo-300/50' : 'border-amber-300/60'}`}
      ...
      <div className={`px-8 py-6 border-b flex items-center gap-4
          ${validationAlert.type === 'GESTANTE' ? 'bg-pink-200/25 border-pink-200/40' : validationAlert.type === 'TRANSFER' ? 'bg-indigo-200/25 border-indigo-200/40' : 'bg-amber-200/30 border-amber-200/50'}`}>
        
        <div className={`p-4 rounded-full shrink-0 border backdrop-blur-sm
            ${validationAlert.type === 'GESTANTE' ? 'bg-pink-300/40 text-pink-700 border-pink-300/50' : validationAlert.type === 'TRANSFER' ? 'bg-indigo-300/40 text-indigo-700 border-indigo-300/50' : 'bg-amber-300/50 text-amber-700 border-amber-300/60'}`}>
        ...
        <div>
          <h3 className={`text-2xl font-black drop-shadow-sm
              ${validationAlert.type === 'GESTANTE' ? 'text-pink-800' : validationAlert.type === 'TRANSFER' ? 'text-indigo-800' : 'text-amber-700'}`}>
              {validationAlert.title}
          </h3>
          <p className={`text-xs font-bold mt-1 uppercase tracking-wide
              ${validationAlert.type === 'GESTANTE' ? 'text-pink-600' : validationAlert.type === 'TRANSFER' ? 'text-indigo-600' : 'text-amber-600'}`}>
      ...
      <div className="p-8 max-h-[60vh] overflow-y-auto">
        ...
        <div className={`p-5 rounded-2xl border flex items-start gap-4 backdrop-blur-sm
            ${validationAlert.type === 'GESTANTE' ? 'bg-pink-100/30 border-pink-300/40' : validationAlert.type === 'TRANSFER' ? 'bg-indigo-100/30 border-indigo-300/40' : 'bg-amber-100/40 border-amber-300/50'}`}>
          <div className={`mt-1
              ${validationAlert.type === 'GESTANTE' ? 'text-pink-600' : validationAlert.type === 'TRANSFER' ? 'text-indigo-600' : 'text-amber-600'}`}>
          ...
          <div className="flex-1">
              ...
              {validationAlert.highlight && (
                  <div className="animate-baile mt-4 inline-block bg-amber-400 text-amber-900 text-xs font-black px-4 py-2 rounded-xl shadow-lg uppercase tracking-wide origin-center border-2 border-amber-500">
                      ⚠ {validationAlert.highlight}
                  </div>
              )}
          </div>
      ...
      <div className="px-8 py-5 bg-white/15 border-t border-white/25 flex justify-end">
         ...
         <button 
            ...
            className={`px-8 py-3 rounded-xl font-black shadow-lg transition-transform active:scale-95 text-sm tracking-wide 
                ${validationAlert.type === 'GESTANTE' ? 'bg-pink-500 hover:bg-pink-600 text-white' : validationAlert.type === 'TRANSFER' ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'bg-amber-400 hover:bg-amber-500 text-amber-900'}`}
        >
